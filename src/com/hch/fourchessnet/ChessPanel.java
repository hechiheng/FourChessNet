...
public class ChessPanel extends View {
	...
	private int chooseChessType;// 选择执棋类型 0未执棋、1执白棋、2执黑棋
	...
	public interface OnChessPanelListener {
		...
		/**
		 * 参数说明：棋子类型(1白棋、2黑棋),棋子i,移动到点i,移动到点j
		 */
	...
	public int getHandChessType() {
		return handChessType;
	}

	public void setHandChessType(int handChessType) {
		this.handChessType = handChessType;
	}

	public int getChooseChessType() {
		return chooseChessType;
	}

	public void setChooseChessType(int chooseChessType) {
		this.chooseChessType = chooseChessType;
	}
	...
	public void restart() {
		initChessIndex();
		invalidate();
		chessPanelListener.onHandTypeChange(handChessType);
	}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		if (event.getAction() == MotionEvent.ACTION_DOWN) {
			...
			if (chooseChessType != handChessType) {
				Toast.makeText(this.getContext(), "等待对方移动棋子", Toast.LENGTH_LONG)
						.show();
				return true;
			}
			for (int i = 0; i < chesss.length; i++) {
				if (chesss[i].isClickChess(x, y)) {
					...
					if (handChessType == 2 && chesss[i].isWhite()) {
						Toast.makeText(this.getContext(), "请选择黑棋",
								Toast.LENGTH_LONG).show();
						return true;
					} else if (handChessType == 1 && !chesss[i].isWhite()) {
						Toast.makeText(this.getContext(), "请选择白棋",
								Toast.LENGTH_LONG).show();
						return true;
					}
					if (chooseChessType == handChessType) {
						...
						if (points[i][j].isClickPoint(x, y)) {
							if (idx_chess_selected >= 0) {
								...
								if ((Math.abs(_x) <= cellWidth && _y == 0)
									...
									chessPanelListener.onMoveChess(chessType,
											idx_chess_selected, i, j);
	...
	public void moveChess(String msg) {
		Log.d("aaa", "-----msg:" + msg);
		int n = 0;
		String[] temp = msg.split(",");
		chessType = Integer.valueOf(temp[n++]);
		int chess_i = Integer.valueOf(temp[n++]);
		int point_i = Integer.valueOf(temp[n++]);
		int point_j = Integer.valueOf(temp[n++]);
		idx_chess_selected = chess_i;
		chesss[idx_chess_selected].setX(points[point_i][point_j].getX()
				- chesss[idx_chess_selected].getWidth() / 2);
		chesss[idx_chess_selected].setY(points[point_i][point_j].getY()
				- chesss[idx_chess_selected].getWidth() / 2);
		chesss[idx_chess_selected].setSelected(false);
		points[point_i][point_j].setChess(chesss[idx_chess_selected]);
		points[chesss[idx_chess_selected].getI_point()][chesss[idx_chess_selected]
				.getJ_point()].setChess(null);
		chesss[idx_chess_selected].setIdx_point(point_i, point_j);

		List<Chess> list = checkEatoff(chesss[idx_chess_selected]);
		for (Chess chess : list) {
			chess.setEatoff(true);
			points[chess.getI_point()][chess.getJ_point()].setChess(null);
			if (chess.isWhite()) {
				whiteChessNum--;
			} else {
				blackChessNum--;
